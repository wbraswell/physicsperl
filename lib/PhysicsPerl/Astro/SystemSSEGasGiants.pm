...
use parent qw(PhysicsPerl::Astro::SystemGasGiants);
use PhysicsPerl::Astro::SystemGasGiants;
...
# [[[ OO PROPERTIES ]]]
our hashref $properties = {};  # DEV NOTE: must not declare 'bodies', to avoid clobbering property inherited from PhysicsPerl::Astro::SystemGasGiants in CPPOPS_CPPTYPES mode

# [[[ SUBROUTINES & OO METHODS ]]]

our void::method $advance_loop = sub {
    ( my PhysicsPerl::Astro::SystemSSEGasGiants $self, my constant_number $delta_time, my constant_integer $time_step_max ) = @ARG;
    ...
    for my integer $time_step ( 0 .. ( $time_step_max - 1 ) ) {
        $k = 0;
#        print {*STDERR} 'in SystemSSEGasGiants::advance_loop(), have initial $k = ', $k, "\n";
...
#                print {*STDERR} 'in SystemSSEGasGiants::advance_loop(), have $k = ', $k, "\n";
