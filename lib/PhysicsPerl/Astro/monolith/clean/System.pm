## no critic qw(Capitalization ProhibitMultiplePackages ProhibitReusedNames RequireFilenameMatchesPackage)
...
our PhysicsPerl::Astro::Body $jupiter = sub {
    ...
    $body->{vx}   = +1.660_076_642_744_036_94e-03
        * PhysicsPerl::Astro::Body::DAYS_PER_YEAR();
    $body->{vy} = +7.699_011_184_197_404_25e-03
        * PhysicsPerl::Astro::Body::DAYS_PER_YEAR();
...
our PhysicsPerl::Astro::Body $saturn = sub {
    ...
    $body->{vx}   = -2.767_425_107_268_624_11e-03
        * PhysicsPerl::Astro::Body::DAYS_PER_YEAR();
    $body->{vy} = +4.998_528_012_349_172_38e-03
        * PhysicsPerl::Astro::Body::DAYS_PER_YEAR();
...
our PhysicsPerl::Astro::Body $uranus = sub {
    ...
    $body->{vx}   = +2.964_601_375_647_616_18e-03
        * PhysicsPerl::Astro::Body::DAYS_PER_YEAR();
    $body->{vy} = +2.378_471_739_594_809_50e-03
        * PhysicsPerl::Astro::Body::DAYS_PER_YEAR();
...
our PhysicsPerl::Astro::Body $neptune = sub {
    ...
    $body->{vx}   = +2.680_677_724_903_893_22e-03
        * PhysicsPerl::Astro::Body::DAYS_PER_YEAR();
    $body->{vy} = +1.628_241_700_382_422_95e-03
        * PhysicsPerl::Astro::Body::DAYS_PER_YEAR();
...
our number::method $energy = sub {
    ...
    for my integer $i ( 0 .. ( ( scalar @{ $self->{bodies} } ) - 1 ) ) {
        ...
        for my integer $j (
            ( $i + 1 ) .. ( ( scalar @{ $self->{bodies} } ) - 1 ) )
        {
            my PhysicsPerl::Astro::Body_raw $body_j
                = $self->{bodies}->[$j]->get_raw();
...
our void::method $advance_loop = sub {
    (   my PhysicsPerl::Astro::System $self,
        my constant_number $delta_time,
        my constant_integer $time_step_max
    ) = @ARG;
    ...
    my constant_sse_number_pair $delta_time_sse
        = constant_sse_number_pair::new_from_singleton_duplicate($delta_time);
    ...
    my constant_sse_number_pair $zero_point_five
        = constant_sse_number_pair::new_from_singleton_duplicate(0.5);
    my constant_sse_number_pair $one_point_five
        = constant_sse_number_pair::new_from_singleton_duplicate(1.5);
    ...
    for my integer $time_step ( 0 .. ( $time_step_max - 1 ) ) {
        ...
        for ( my integer $i = 0; $i < $bodies_size_triangle; $i += 2 ) {
            ...
            $distance_squared = ( $dx sse_mul $dx) sse_add( $dy sse_mul $dy)
                sse_add( $dz sse_mul $dz);
            $distance = sse_recip_sqrt_32bit_on_64bit($distance_squared);
            $distance
                = $distance sse_mul $one_point_five sse_sub(
                ( $zero_point_five sse_mul $distance_squared) sse_mul $distance)
                sse_mul( $distance sse_mul $distance);
            $distance
                = $distance sse_mul $one_point_five sse_sub(
                ( $zero_point_five sse_mul $distance_squared) sse_mul $distance)
                sse_mul( $distance sse_mul $distance);
            $magnitude = ( $delta_time_sse sse_div $distance_squared)
                sse_mul $distance;
