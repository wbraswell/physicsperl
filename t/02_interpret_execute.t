...
find(
    sub {
        ...
        #        RPerl::diag('in 02_interpret_execute.t, have $file = ' . $file . "\n");
...
for my $test_file ( sort keys %{$test_files} ) {
    ...
    #    RPerl::diag( 'in 02_interpret_execute.t, have $test_file = ' . $test_file . "\n" );
    my $pid;
    ...
    #        if ( $stdout_select->can_read(0) )  { RPerl::diag('in 02_interpret_execute.t, can read STDOUT_TEST for $test_file = ' . $test_file . "\n"); }
    #        if ( $stderr_select->can_read(0) )  { RPerl::diag('in 02_interpret_execute.t, can read STDERR_TEST for $test_file = ' . $test_file . "\n"); }
    ...
    #    RPerl::diag( 'in 02_interpret_execute.t, have $CHILD_ERROR = ' . $CHILD_ERROR . "\n" );
    #    RPerl::diag( 'in 02_interpret_execute.t, have $test_exit_status = ' . $test_exit_status . "\n" );
...
#                RPerl::diag( 'in 02_interpret_execute.t, before foreach loop, have successes =' . "\n" . Dumper($test_files->{$test_file}->{successes}) . "\n" );
           ...
           #                    RPerl::diag( 'in 02_interpret_execute.t, top of foreach loop, have $success = ' . $success . "\n" );
           #                    RPerl::diag( 'in 02_interpret_execute.t, top of foreach loop, have $stdout_generated_line = ' . $stdout_generated_line . "\n" );
           # each stdout line is only allowed to match one success string
                    if ( $stdout_generated_line =~ /\Q$success\E/xms ) {

                        #                        RPerl::diag( 'in 02_interpret_execute.t, MATCH' . "\n" );
                    ...
                    #                    else { RPerl::diag( 'in 02_interpret_execute.t, NO MATCH' . "\n" ); }
                }
            }
#            RPerl::verbose( 'in 02_interpret_execute.t, have missing successes =' . "\n" . Dumper( $test_files->{$test_file}->{successes} ) . "\n" );
...
#                }
#            }
#            RPerl::verbose( 'in 02_interpret_execute.t, have $missing_successes =' . "\n" . Dumper($missing_successes) . "\n" );
